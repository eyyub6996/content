...
def f_thales():
    f, ax = fig(4.2, 2.8)
    A = np.array((0, 0)); B = np.array((4.5, 2.6)); C = np.array((5, -0.4))
    k = 0.55
    M_ = A + k * (B - A); N_ = A + k * (C - A)
    ax.plot(*zip(A, B), color=NAVY, lw=1.6); ax.plot(*zip(A, C), color=NAVY, lw=1.6)
    ax.plot(*zip(B, C), color=RED, lw=1.8); ax.plot(*zip(M_, N_), color=GREEN, lw=1.8)
    for P, n, dx, dy in ((A, "A", -0.25, 0), (B, "B", 0.1, 0.1), (C, "C", 0.12, -0.15), (M_, "M", -0.1, 0.18), (N_, "N", 0.05, -0.28)):
        ax.plot(*P, "o", color="#333", ms=4); ax.text(P[0] + dx, P[1] + dy, n, fontsize=11)
    ax.text(3.9, -0.95, r"$(MN) \parallel (BC)$ : $\frac{AM}{AB} = \frac{AN}{AC} = \frac{MN}{BC}$", fontsize=10, ha="center", color=NAVY)
    ax.set_xlim(-0.6, 6); ax.set_ylim(-1.2, 3); ax.set_aspect("equal"); ax.axis("off")
    return _svg(f)


def f_pythagore_crpe():
    f, ax = fig(3.4, 2.6)
    A, B, C = (0, 0), (4, 0), (0, 3)
    ax.add_patch(Polygon([A, B, C], closed=True, facecolor="#eef4fb", edgecolor=NAVY, lw=1.6))
    ax.add_patch(Rectangle((0, 0), 0.3, 0.3, fill=False, edgecolor=NAVY, lw=1))
    ax.text(2, -0.35, "$a = 4$", ha="center", fontsize=11, color=GREEN)
    ax.text(-0.45, 1.5, "$b = 3$", ha="center", fontsize=11, color=ORANGE, rotation=90)
    ax.text(2.25, 1.75, "$c = 5$", fontsize=11, color=VIOLET)
    ax.text(2, -1.0, r"$c^2 = a^2 + b^2$ : $25 = 16 + 9$", ha="center", fontsize=10.5, color=NAVY)
    ax.set_xlim(-0.9, 4.6); ax.set_ylim(-1.3, 3.3); ax.set_aspect("equal"); ax.axis("off")
    return _svg(f)
...
def f_symetrie():
    ...
    P = np.array([[-3, 0.5], [-1, 0.5], [-1.5, 2.2], [-2.6, 1.6]])
    ax.add_patch(Polygon(P, closed=True, facecolor="#9cc3e6", edgecolor=NAVY, lw=1.4))
    Q = P * np.array([-1, 1])
    ax.add_patch(Polygon(Q, closed=True, facecolor="#f5d49a", edgecolor=ORANGE, lw=1.4))
    for p, q in zip(P, Q):
        ax.plot([p[0], q[0]], [p[1], q[1]], ":", color="#999", lw=0.8)
    ax.text(0.1, 2.5, "axe", color=RED, fontsize=9)
    ax.set_xlim(-3.4, 3.4); ax.set_ylim(0, 2.8); ax.set_aspect("equal"); ax.axis("off")
    return _svg(f)


def f_solides():
    f, axs = plt.subplots(1, 4, figsize=(7.0, 1.9))
    ax = axs[0]
    fr = [(0, 0), (2, 0), (2, 1.2), (0, 1.2)]; o = np.array((0.7, 0.5))
    ax.add_patch(Polygon(fr, closed=True, fill=False, edgecolor=NAVY, lw=1.4))
    back = [np.array(p) + o for p in fr]
    ax.add_patch(Polygon(back, closed=True, fill=False, edgecolor=NAVY, lw=1, ls="--"))
    for p, q in zip(fr, back):
        ax.plot([p[0], q[0]], [p[1], q[1]], color=NAVY, lw=1)
    ax.set_title("pavé : $V = L \\times \\ell \\times h$", fontsize=10)
    ax = axs[1]
    t = np.linspace(0, 2 * np.pi, 100)
    ax.plot(np.cos(t), 0.3 * np.sin(t), color=NAVY); ax.plot(np.cos(t), 2 + 0.3 * np.sin(t), color=NAVY)
    ax.plot([-1, -1], [0, 2], color=NAVY); ax.plot([1, 1], [0, 2], color=NAVY)
    ax.set_title("cylindre : $V = \\pi r^2 h$", fontsize=10)
    ax = axs[2]
    ax.plot(np.cos(t), 0.3 * np.sin(t), color=NAVY); ax.plot([-1, 0, 1], [0, 2, 0], color=NAVY)
    ax.set_title("cône : $V = \\frac{1}{3}\\pi r^2 h$", fontsize=10)
    ax = axs[3]
    ax.add_patch(Circle((0, 1), 1, fill=False, edgecolor=NAVY, lw=1.4)); ax.plot(np.cos(t), 1 + 0.3 * np.sin(t), color=NAVY, ls="--", lw=0.8)
    ax.set_title("boule : $V = \\frac{4}{3}\\pi r^3$", fontsize=10)
    for a in axs:
        a.set_aspect("equal"); a.axis("off")
    axs[0].set_xlim(-0.2, 3); axs[0].set_ylim(-0.3, 2.2)
    for a in axs[1:]:
        a.set_xlim(-1.3, 1.3); a.set_ylim(-0.5, 2.3)
    f.tight_layout()
    return _svg(f)
...
def f_diagramme():
    f, ax = fig(4.2, 2.4)
    notes = [8, 9, 10, 11, 12, 13, 14]; eff = [2, 3, 5, 6, 4, 3, 1]
    ax.bar(notes, eff, color=BLUE, width=0.6)
    ax.set_xticks(notes); ax.set_ylabel("effectif", fontsize=9); ax.set_xlabel("note", fontsize=9)
...
def f_affine():
    f, ax = fig(3.6, 2.8)
    axes(ax, (-1.5, 4.5), (-2.5, 6), xticks=[1, 2, 3], yticks=[2, 4])
    xs = np.linspace(-1.5, 4.2, 10)
    ax.plot(xs, 1.5 * xs, color=GREEN, lw=2, label="linéaire $f(x) = 1{,}5x$")
    ax.plot(xs, -xs + 3, color=RED, lw=2, label="affine $g(x) = -x + 3$")
    ax.plot(0, 3, "o", color=RED, ms=4)
...
def f_frise():
    f, ax = fig(7.0, 1.9)
    periods = [("Préhistoire", "#e8e1d0"), ("Antiquité", "#f5d49a"), ("Moyen Âge", "#b9e0c4"), ("Temps\nmodernes", "#9cc3e6"), ("Époque\ncontemporaine", "#f2b8b5")]
    for i, (n, c) in enumerate(periods):
        ax.add_patch(Rectangle((i, 0), 1, 0.6, facecolor=c, edgecolor="#555", lw=0.8))
        ax.text(i + 0.5, 0.3, n, ha="center", va="center", fontsize=8.5)
    bounds = [(1, "vers −3300\ninvention\nde l'écriture"), (2, "476\nchute de\nl'Empire romain\nd'Occident"), (3, "1492\nC. Colomb\nen Amérique"), (4, "1789\nRévolution\nfrançaise")]
    for x, lab in bounds:
        ax.plot([x, x], [-0.05, 0.65], color="#333", lw=1.2)
        ax.text(x, -0.1, lab, ha="center", va="top", fontsize=7)
    ax.annotate("", xy=(5.15, 0.3), xytext=(4.95, 0.3), arrowprops=dict(arrowstyle="-|>", color="#555"))
    ax.set_xlim(-0.1, 5.25); ax.set_ylim(-1.25, 0.75); ax.axis("off")
    return _svg(f)


def f_circuit():
    f, ax = fig(3.6, 2.4)
    ax.plot([0, 4, 4, 0, 0], [0, 0, 2.4, 2.4, 0], color="#333", lw=1.5)
    ax.add_patch(Rectangle((-0.25, 0.8), 0.5, 0.8, facecolor="white", edgecolor="#333", lw=1.5, zorder=3))
    ax.plot([-0.2, 0.2], [1.35, 1.35], color="#333", lw=3, zorder=4); ax.plot([-0.1, 0.1], [1.05, 1.05], color="#333", lw=1.5, zorder=4)
    ax.text(-0.45, 1.2, "pile", ha="right", fontsize=9)
    ax.add_patch(Circle((2, 2.4), 0.3, facecolor="white", edgecolor="#333", lw=1.5, zorder=3))
    ax.plot([1.8, 2.2], [2.2, 2.6], color="#333", lw=1.2, zorder=4); ax.plot([1.8, 2.2], [2.6, 2.2], color="#333", lw=1.2, zorder=4)
    ax.text(2, 2.85, "lampe", ha="center", fontsize=9)
    ax.plot([1.5, 2.4], [0, 0], color="white", lw=4, zorder=3)
    ax.plot([1.5, 2.35], [0, 0.4], color="#333", lw=1.5, zorder=4)
    ax.plot([1.5, 2.5], [0, 0], "o", color="#333", ms=4, zorder=5)
    ax.text(2, -0.35, "interrupteur ouvert", ha="center", fontsize=9)
    ax.set_xlim(-1.2, 4.5); ax.set_ylim(-0.7, 3.2); ax.set_aspect("equal"); ax.axis("off")
    return _svg(f)
...
def f_saisons():
    ...
    ax.plot(3 * np.cos(t), 1.1 * np.sin(t), color="#aaa", lw=1)
    ax.add_patch(Circle((0, 0), 0.45, color="#f5b942"))
    ax.text(0, -0.75, "Soleil", ha="center", fontsize=8.5)
    for ang, lab in ((0, "solstice de\ndécembre\n(hiver, hém. N)"), (np.pi, "solstice\nde juin\n(été, hém. N)")):
        x, y = 3 * np.cos(ang), 1.1 * np.sin(ang)
        ax.add_patch(Circle((x, y), 0.25, color=BLUE))
        d = 0.45
        ax.plot([x - d * np.sin(np.radians(23)), x + d * np.sin(np.radians(23))], [y - d * np.cos(np.radians(23)), y + d * np.cos(np.radians(23))], color=RED, lw=1.2)
        ax.text(x, y - 0.55, lab, ha="center", va="top", fontsize=7)
    ax.text(0, 1.3, "axe de la Terre incliné de 23° : il garde la même direction", ha="center", fontsize=7.5, color=RED)
    ax.set_xlim(-4, 4); ax.set_ylim(-2.2, 1.6); ax.set_aspect("equal"); ax.axis("off")
    return _svg(f)


def f_angles():
    f, ax = fig(3.8, 2.6)
    ax.plot([-0.5, 5], [0, 0], color=NAVY, lw=1.5); ax.plot([-0.5, 5], [2, 2], color=NAVY, lw=1.5)
    ax.plot([0.6, 3.6], [-0.6, 2.6], color=RED, lw=1.5)
    # intersections: y=0 -> x=1.15 ; y=2 -> x=3.15
    from matplotlib.patches import Arc
    ax.add_patch(Arc((1.15, 0), 0.8, 0.8, theta1=0, theta2=45, color=GREEN, lw=1.6))
    ax.add_patch(Arc((3.15, 2), 0.8, 0.8, theta1=180, theta2=225, color=GREEN, lw=1.6))
    ax.add_patch(Arc((3.15, 2), 0.6, 0.6, theta1=0, theta2=45, color=ORANGE, lw=1.6))
    ax.text(1.65, 0.12, "a", color=GREEN, fontsize=11); ax.text(2.45, 1.62, "b", color=GREEN, fontsize=11)
    ax.text(3.55, 2.12, "c", color=ORANGE, fontsize=11)
    ax.text(5.05, 0, "(d)", fontsize=9, va="center"); ax.text(5.05, 2, "(d')", fontsize=9, va="center")
    ax.text(2.5, -0.9, "(d) // (d') : a = b (alternes-internes), a = c (correspondants)", fontsize=8, ha="center", color=NAVY)
    ax.set_xlim(-0.6, 5.6); ax.set_ylim(-1.1, 2.8); ax.set_aspect("equal"); ax.axis("off")
    return _svg(f)
...
def f_trigo():
    ...
    ax.add_patch(Rectangle((3.7, 0), 0.3, 0.3, fill=False, edgecolor=NAVY, lw=1))
    from matplotlib.patches import Arc
    ax.add_patch(Arc(A, 1.2, 1.2, theta1=0, theta2=33, color=RED, lw=1.6))
    ax.text(0.7, 0.12, "$\\alpha$", color=RED, fontsize=11)
    ax.text(2, -0.35, "côté adjacent à $\\alpha$", ha="center", fontsize=9, color=GREEN)
    ax.text(4.15, 1.3, "côté\nopposé", ha="left", va="center", fontsize=9, color=ORANGE)
    ax.text(1.7, 1.55, "hypoténuse", ha="center", fontsize=9, color=VIOLET, rotation=33)
    ax.text(-0.2, -0.15, "A", fontsize=10); ax.text(4.05, -0.2, "B", fontsize=10); ax.text(4.05, 2.65, "C", fontsize=10)
    ax.set_xlim(-0.4, 5.4); ax.set_ylim(-0.6, 3.0); ax.set_aspect("equal"); ax.axis("off")
    return _svg(f)
...
def f_patron():
    ...
    for (i, j) in cells:
        ax.add_patch(Rectangle((i, j), 1, 1, facecolor="#f5d49a" if (i, j) == (1, 1) else "#eef4fb", edgecolor=NAVY, lw=1.4))
    ax.text(1.5, 1.5, "base", ha="center", va="center", fontsize=8)
    ax.set_xlim(-0.2, 4.2); ax.set_ylim(-0.2, 3.2); ax.set_aspect("equal"); ax.axis("off")
    return _svg(f)


def f_transfos():
    f, axs = plt.subplots(1, 3, figsize=(7.0, 2.2))
    tri = np.array([[0, 0], [1.2, 0], [0.3, 0.9]])
    # translation
    ax = axs[0]
    ax.add_patch(Polygon(tri, closed=True, facecolor="#9cc3e6", edgecolor=NAVY))
    ax.add_patch(Polygon(tri + [2, 0.8], closed=True, facecolor="#f5d49a", edgecolor=ORANGE))
    ax.annotate("", xy=(2, 0.8), xytext=(0, 0), arrowprops=dict(arrowstyle="-|>", color=RED))
    ax.set_title("translation", fontsize=10)
    ax.set_xlim(-0.3, 3.5); ax.set_ylim(-0.3, 2)
    # rotation 90° autour de O
    ax = axs[1]
    t2 = tri + [0.4, 0.2]
    R = np.array([[0, -1], [1, 0]])
    ax.add_patch(Polygon(t2, closed=True, facecolor="#9cc3e6", edgecolor=NAVY))
    ax.add_patch(Polygon(t2 @ R.T, closed=True, facecolor="#f5d49a", edgecolor=ORANGE))
    ax.plot(0, 0, "o", color=RED, ms=4); ax.text(0.05, -0.3, "O", fontsize=9)
    ax.set_title("rotation (centre O, 90°)", fontsize=10)
    ax.set_xlim(-1.4, 1.9); ax.set_ylim(-0.5, 1.9)
    # homothétie
    ax = axs[2]
    ax.add_patch(Polygon(tri + [0.3, 0.1], closed=True, facecolor="#9cc3e6", edgecolor=NAVY))
    ax.add_patch(Polygon(2 * (tri + [0.3, 0.1]), closed=True, facecolor="none", edgecolor=ORANGE, lw=1.6))
    ax.plot(0, 0, "o", color=RED, ms=4); ax.text(-0.2, -0.3, "O", fontsize=9)
    for P in tri + [0.3, 0.1]:
        ax.plot([0, 2 * P[0]], [0, 2 * P[1]], ":", color="#999", lw=0.8)
    ax.set_title("homothétie (centre O, k = 2)", fontsize=10)
