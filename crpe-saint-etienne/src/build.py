#!/usr/bin/env python3
"""Construit le PDF « Réussir le CRPE 2027 — méthode complète ».
...
PDF_NAME = "CRPE_2027_Saint-Etienne_methode_complete.pdf"
FILES = [
    "00_debut.txt",
    "01_grammaire.txt",
    "02_lexique_redaction.txt",
    "03_nombres.txt",
    "04_proportionnalite_algebre.txt",
    "05_geometrie_mesures.txt",
    "06_donnees_algo.txt",
    "07_hg_emc.txt",
    "08_sciences.txt",
    "09_arts_anglais.txt",
    "10_oraux.txt",
    "11_sujets_blancs.txt",
    "12_annexes.txt",
]

STARS = {1: "★☆☆☆", 2: "★★☆☆", 3: "★★★☆", 4: "★★★★"}
LEVEL = {1: "facile", 2: "moyen", 3: "difficile", 4: "niveau concours"}
...
def inline(s):
    ...
    s = re.sub(r"`([^`]+)`", keep, s)
    res = _inline(s)
    return re.sub(r"\x00(\d+)\x00", lambda m: "<code>" + esc(codes[int(m.group(1))]) + "</code>", res)
...
<title>Réussir le CRPE 2027 — méthode complète</title>
...
def main():
    ...
    doc.set_metadata(
        {
            "title": "Réussir le CRPE 2027 — méthode complète (académie de Lyon, Saint-Étienne)",
            "subject": "Cours, exercices corrigés, sujets blancs, planning daté jusqu'aux écrits du CRPE 2027",
            "author": "",
            "creator": "crpe-saint-etienne",
